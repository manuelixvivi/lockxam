...
def test_import_class_structure_all_valid(client: TestClient, api_test_data, db: Session):
    ...
    db.add(cls)
    # 2. Subject
    subj = Subject(school_id=school_a.id, code="MAT-10", name="Matematika Wajib Kelas 10", is_active=True)
    db.add(subj)
    db.commit()
    ...
    cst = db.query(ClassSubjectTeacher).filter_by(class_id=cls.id, subject_id=subj.id, teacher_id=teacher.id).first()
...
def test_import_class_structure_wrong_academic_year(client: TestClient, api_test_data, db: Session):
    ...
    year_b = AcademicYear(school_id=school_b.id, name="2027/2028", start_date=datetime.now(), end_date=datetime.now(), status="ACTIVE")
    db.add(year_b)
    db.commit()

    payload = {
        "academic_year_id": year_b.id,  # School B
        "classes": []
    }
...
def test_import_class_structure_cross_tenant_subject(client: TestClient, api_test_data, db: Session):
    ...
    db.add(cls)
    # Subject in School B
    subj_b = Subject(school_id=school_b.id, code="MAT-10-B", name="Matematika School B", is_active=True)
    db.add(subj_b)
    db.commit()
...
def test_import_class_structure_cross_tenant_teacher(client: TestClient, api_test_data, db: Session):
...
def test_import_class_structure_duplicate_subject_in_file(client: TestClient, api_test_data, db: Session):
...
def test_import_class_structure_duplicate_cst_in_file(client: TestClient, api_test_data, db: Session):
...
def test_import_class_structure_existing_class_subject(client: TestClient, api_test_data, db: Session):
...
def test_import_class_structure_existing_cst(client: TestClient, api_test_data, db: Session):
    ...
    cs = ClassSubject(school_id=school_a.id, class_id=cls.id, subject_id=subj.id)
    db.add(cs)
    cst = ClassSubjectTeacher(school_id=school_a.id, class_id=cls.id, subject_id=subj.id, teacher_id=teacher.id)
    db.add(cst)
    db.commit()
...
def test_import_class_structure_mixed_valid_invalid(client: TestClient, api_test_data, db: Session):
    ...
    cls1 = ClassEntity(school_id=school_a.id, academic_year_id=year_2025.id, name="X-MIPA-1", grade_level="10")
    db.add(cls1)
    ...
    cls2 = ClassEntity(school_id=school_a.id, academic_year_id=year_2025.id, name="X-MIPA-2", grade_level="10")
    db.add(cls2)
    db.commit()
...
def test_import_class_structure_db_failure_rollback(client: TestClient, api_test_data, db: Session, monkeypatch):
    ...
    from app.repositories.academic.class_subject_teacher_repository import ClassSubjectTeacherRepository
