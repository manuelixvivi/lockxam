...
class SchoolStaffService:
    ...
    def list_teachers(db: Session, school_id: int) -> list[AuthAccount]:
        ...
        for t in teachers:
            ...
            if combined_subject_ids:
                ...
                relational_subject_names = [s.name for s in relational_subjects if s.school_id == school_id]
    ...
    def update_teacher(
        ...
        teacher = auth_repository.get_by_public_id_and_school(db, school_id, teacher_public_id, role="TEACHER")
        if not teacher:
