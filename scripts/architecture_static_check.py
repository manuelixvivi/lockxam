...
def check_file_does_not_contain(filepath, pattern, error_message, repo_exemption=False):
    ...
    with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
        ...
        for line_num, line in enumerate(f, 1):
            ...
            if in_docstring or sline.startswith("#") or sline.startswith("//") or sline.startswith("*"):
                continue

            if re.search(pattern, line):
                if repo_exemption and re.search(repo_call_pattern, line):
                    continue
                print(f"[FAIL] {error_message} found at line {line_num} in {os.path.relpath(filepath, BASE_DIR)}: {sline}")
                passed = False
    return passed
...
def run_checks():
        ...
        for file in files:
            if file.endswith(".py"):
                ...
                for pat, msg in db_access_patterns:
                    ...
                    if not check_file_does_not_contain(fp, pat, f"{msg} in Service Layer", repo_exemption=is_exempt_op):
                        passed = False
    ...
    for root, _, files in os.walk(exam_svc_dir):
        for file in files:
            if file.endswith(".py"):
                fp = os.path.join(root, file)
                if not check_file_does_not_contain(fp, r"app\.repositories\.academic", "Cross-domain import from academic repositories"):
                    passed = False
                if not check_file_does_not_contain(fp, r"app\.models\.academic", "Cross-domain import from academic models"):
                    passed = False
                if not check_file_does_not_contain(fp, r"app\.models\.teacher", "Cross-domain import from teacher models"):
                    passed = False
    ...
    for root, _, files in os.walk(frontend_src):
        for file in files:
            if file.endswith((".ts", ".tsx")):
                fp = os.path.join(root, file)
                if not check_file_does_not_contain(fp, r"localStorage.*access_token", "localStorage access_token usage"):
                    passed = False
                if not check_file_does_not_contain(fp, r"localStorage.*refresh_token", "localStorage refresh_token usage"):
                    passed = False

    # 5. Zero token_default, uploadAnswerImage, or BROADCAST_STORE
    for root, _, files in os.walk(BASE_DIR):
        if "node_modules" in root or ".git" in root or ".venv" in root or "scripts" in root: continue
        for file in files:
            if file.endswith((".ts", ".tsx", ".py")):
                fp = os.path.join(root, file)
                if not check_file_does_not_contain(fp, r"token_default", "Hardcoded fallback token 'token_default'"):
                    passed = False
                if not check_file_does_not_contain(fp, r"uploadAnswerImage|upload-answer-image", "Unfrozen uploadAnswerImage feature"):
                    passed = False
                if not check_file_does_not_contain(fp, r"BROADCAST_STORE", "In-memory BROADCAST_STORE dict"):
                    passed = False

    # 6. Zero mockQuestions in StudentCbtEngineView.tsx
    cbt_view = os.path.join(BASE_DIR, "frontend", "src", "views", "student", "StudentCbtEngineView.tsx")
        ...
        with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
            ...
            for line_num, line in enumerate(f, 1):
                ...
                if write_pat.search(line):
                    ...
                    print(f"[FAIL] Active write to teacher.subjects_taught (TeacherSubject authority violation) "
    ...
    for root, _, files in os.walk(os.path.join(BASE_DIR, "app", "api")):
        for file in files:
            if file.endswith(".py"):
                if not check_no_subjects_taught_write(os.path.join(root, file), allowed_function_context="__NONE__"):
                    passed = False
        ...
        with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
            for line_num, line in enumerate(f, 1):
                ...
                if in_request_cls and field_pat.match(line):
                    rel = os.path.relpath(filepath, BASE_DIR)
                    print(f"[FAIL] Writable subjects_taught field in request schema at line {line_num} in {rel}: {sline}")
                    ok = False
        return ok
